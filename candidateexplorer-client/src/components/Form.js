...
const Form = (props) => {
  ...
  const loadData = async() =>{
    ...
    setEnteredValue({ enteredName:"",enteredEmail:""});
    }).catch((error)=> 
...
if(enteredNameIsValid && enteredEmailIsValid && enteredContactIsValid && enteredSkillsIsValid
   ...
   && enteredCurrentCompanyIsValid && enteredFeedbackIsValid
       ){
        formIsValid=true;
        }
...
<div className={feedbackInputclasses}>
        <label htmlFor='text'>Interview Feedback</label>
        <select 
          ...
          <option  value="3rd Round">Third Round</option>
        </select>
    </div>

{feedbackInputHasError && <p className="error-text">Interview Feedback must not be empty</p>}
