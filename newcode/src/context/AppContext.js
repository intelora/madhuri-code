...
export const AppcontextProvider = ({ children }) => {
    ...
    const connectWallet = async (switchChain) => {
        try {
            ...
            if (
                switchChain &&
                +networkId !== +siteConfig.NETWORK.ID &&
                web3Modal.cachedProvider
            ) {
                await handleSwitchChain(siteConfig.NETWORK.ID, provider)
            }
            ...
            if (network.chainId === siteConfig.NETWORK.ID) {
                ...
                return Promise.reject({
                    message: `Please change the network to ${siteConfig.NETWORK.NAME}.`,
                })
            }
        } catch (error) {
            console.log(error)
